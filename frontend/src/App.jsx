...
function App() {
  ...
  // Load topics and quizzes from server
  useEffect(() => {
    ...
    // Load sample local quiz
    axios.get("/sampleQuizFile.json")
      .then(res => {
        const data = res.data;
  ...
  useEffect(() => {
    if (selectedQuiz) {
      axios.get(`http://localhost:3000/api/questions/${selectedQuiz}`)
        .then(res => {
  ...
  return (
    <div style={{
      ...
      <h1 style={{ color: "#2563eb", fontSize: "2.5rem", marginBottom: "30px" }}>
        🎓 Mock Quiz App
      </h1>
      ...
      {selectedQuiz && !quizFinished && questions.length > 0 && (
        <section>
          <h2 style={{ color: "#333" }}>Question {currentQIndex + 1} of {questions.length}</h2>
